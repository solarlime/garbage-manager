...
export function render(list: HTMLUListElement, item: Data): HTMLLIElement {
  const newItem = document.createElement('li');
  newItem.className = 'app-view-list-item';

  let parsedContent: string | Blob;
  if (typeof item.content === 'string') {
    // Lists contain new lines' symbols. Parsing them
    const enter = '\n';
    parsedContent = item.content.includes(enter) ? item.content.split(enter).join('<br>&nbsp;') : item.content;
  } else {
    parsedContent = item.content;
  }

  newItem.insertAdjacentHTML('beforeend', '<div class="list-item">\n'
    + '  <div class="item-icon"></div>\n'
    + `    <div class="item-description">${parsedContent}</div>\n`
    + '  <div class="item-delete">\n'
    + `    <input type="checkbox" id="${item.id}" class="item-delete-spoiler">\n`
    + `    <label for="${item.id}" class="item-delete-label"><img class="item-delete-button-svg" src="/src/img/next.png" alt="Show / hide a delete button"></label>\n`
    + '    <div class="item-delete-button">\n'
    + '      <svg class="item-delete-button-svg" viewBox="-47 0 512 512" xmlns="http://www.w3.org/2000/svg">\n'
 ...
 * @param string
 */
export function formsString(forms: HTMLCollectionOf<HTMLFormElement>, string: string):
